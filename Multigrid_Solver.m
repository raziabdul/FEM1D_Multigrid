...
% bugs
	...
	if (dim == 1) 
		PR_coef  = 2;   % For Prolongaton - Restriction
		direct_n = 16;  % size limit of the problem for solving with direct solver
	end
	
	% Generate coefficient matrices and restriction operators of each level at once
	tic;
	if (dim == 1)
		[A_list, R_list, max_level] = Multigrid1D_Vcycle_GenMat(A, direct_n, isTwo);
	end
